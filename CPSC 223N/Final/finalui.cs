...
public class RicochetBall : Form {
    ...
    public RicochetBall() {
        ...
        title.Size = new Size(770, 50);
        title.Location = new Point((formWidth - title.Width) / 2, (titleHeight / 2) - (title.Height / 2));
        title.Text = "CPSC 223N Final Exam by Kassandra Sanchez";
    ...
    protected void initClick(Object sender, EventArgs events) {
        ...
        showBall = true;
        ballPanel.displayBall(showBall);
        // reverts it back to the og color
        ballPanel.changeColor(Color.Red);
        // so the color sequence doesnt jump from red to indigo or something
        skin = 0;
    ...
    protected void updateBallCoords(System.Object sender, ElapsedEventArgs events) {
        ...
        // if it collids with any of the walls, itll change color
        if ((int)System.Math.Round(ballCenterCurrCoordsX + ballRadius) >= this.ClientSize.Width ||
            (int)System.Math.Round(ballCenterCurrCoordsX - ballRadius) <= 0 ||
            (int)System.Math.Round(ballCenterCurrCoordsY - ballRadius) <= 0 ||
            (int)System.Math.Round(ballCenterCurrCoordsY + ballRadius) >= ballHeight) {
            skin++;
            // 0 = Red, 1 = Lime, 2 = Orange, 3 = Indigo
            if (skin == 1) {
                ballPanel.changeColor(Color.Lime);
            } else if (skin == 2) {
                ballPanel.changeColor(Color.Orange);
            } else if (skin == 3) {
                ballPanel.changeColor(Color.Indigo);
            } else {
                ballPanel.changeColor(Color.Red);
                skin = 0;
            }
    ...
    public class GraphicPanel : Panel {
        ...
        private SolidBrush colorfulBrush;
        ...
        public void changeColor(Color newSkin) {
            ballColor = newSkin;
            colorfulBrush.Color = ballColor;
        }
