...
public class RicochetBall : Form {
    ...
    private static double noSnapX;
    private static double noSnapY;
    private static bool hitButton = false;
    ...
    private void textFilled(object sender, EventArgs events) {
        ...
        if (Double.TryParse(enterSpeed.Text, out speed1) == false ||
            ...
            //plus.Enabled = true;
            //minus.Enabled = true;
            enterCurrSpeed.Text = enterSpeed.Text;
    ...
    protected void initClick(Object sender, EventArgs events) {
        ...
        showBall = true;
        ballPanel.displayBall(showBall);
        // This prevents the user from accidently crashing the program
        System.Console.WriteLine("Press the +10 or -10 button to see the speed change before pressing the Start button.");
    ...
    protected void plusClick(Object sender, EventArgs events) {
        userBallSpeed += 10;
        enterCurrSpeed.Text = userBallSpeed.ToString();

        noSnapX = Math.Sign(ballDeltaX);
        noSnapY = Math.Sign(ballDeltaY);

        pixelPerTic = userBallSpeed/ballClockRate;
        ballDeltaX = noSnapX * pixelPerTic * Math.Abs(Math.Cos(userBallDirection * Math.PI/180.0));
        ballDeltaY = noSnapY * pixelPerTic * Math.Abs(Math.Sin(userBallDirection * Math.PI/180.0));
        minus.Enabled = true;
    ...
    protected void minusClick(Object sender, EventArgs events) {
        if (userBallSpeed - 10 < 1) {
            System.Console.WriteLine("Can't make it slower.");
            minus.Enabled = false;
            return;
        }
